...
/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
   SVG ICON SYSTEM â€” inline Lucide-style icons, no emoji
â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
const ICONS = {
  // Nav
...
/* â”€â”€ BOTTOM TAB BAR (mobile nav) â”€â”€ */
.bottom-nav{
  display:none;
...
/* â”€â”€ MOBILE â‰¤768px â€” core layout switch â”€â”€ */
...
/* â”€â”€ MOBILE â‰¤768px â€” mobile-first redesign â”€â”€ */
@media(max-width:768px){
  .sidebar{display:none !important}
  .bottom-nav{display:block}
  .mobile-header{display:flex}
  ...
  /* All grids â†’ single column with generous gap */
  .g2,.g3,.g4,.g23,.g32{
    grid-template-columns:1fr !important;
    gap:12px;
  }

  /* Cards â€” more breathing room */
  .card{padding:18px;border-radius:18px}
  .card-val{font-size:30px}
  .card-lbl{font-size:9px}
  .card-sub{font-size:12px}

  /* Topbar right â€” smaller gap */
  .topbar-right{gap:8px}
  .topbar-right .btn{font-size:12px;padding:8px 12px}

  /* Inputs â€” larger tap targets */
  ...
  /* Buttons â€” full width on mobile forms */
  ...
  /* Panel (coach detail) â€” full-screen bottom sheet */
  .panel{
  ...
  /* Data table â€” horizontal scroll */
  ...
  /* Profile pic overlay â€” always visible on mobile */
...
const TRANSLATIONS = {
  en: {
    ...
    confirm:"Confirm", loading:"Loadingâ€¦", retry:"Retry",
  ...
  fr: {
    ...
    confirm:"Confirmer", loading:"Chargementâ€¦", retry:"RÃ©essayer",
  ...
  es: {
    ...
    confirm:"Confirmar", loading:"Cargandoâ€¦", retry:"Reintentar",
  ...
  de: {
    ...
    confirm:"BestÃ¤tigen", loading:"Ladenâ€¦", retry:"Erneut versuchen",
  ...
  hi: {
    ...
    confirm:"à¤ªà¥à¤·à¥à¤Ÿà¤¿ à¤•à¤°à¥‡à¤‚", loading:"à¤²à¥‹à¤¡ à¤¹à¥‹ à¤°à¤¹à¤¾ à¤¹à¥ˆâ€¦", retry:"à¤ªà¥à¤¨à¤ƒ à¤ªà¥à¤°à¤¯à¤¾à¤¸",
...
function getTier(score){
  if(score==null)return{label:"â€”",cls:"tier-mod",color:"#D4D4D4"};
...
function Ring({score,size=120,stroke=8}){
  if(score==null)return(
    <div style={{width:size,height:size,borderRadius:"50%",background:"var(--bg3)",display:"flex",alignItems:"center",justifyContent:"center"}}>
      <span style={{fontFamily:"'IBM Plex Mono'",fontSize:10,color:"var(--text3)"}}>â€”</span>
    </div>
  );
  ...
  return(
    <div className="land">
      ...
      <section className="land-hero">
        ...
        <p className="land-sub">Real-time athlete monitoring, readiness tracking, injury prevention, and coach intelligence â€” all in one platform.</p>
        <div className="land-btns">
          <button className="land-btn-p" onClick={onGetStarted}>Get Started Free</button>
          <button className="land-btn-g" onClick={onGetStarted}>Coach Dashboard â†’</button>
        </div>
        <div className="land-mockup">
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:10}}>
            {[["AVG READINESS","â€”","#FFD600"],["ATHLETES READY","â€”","#22C55E"],["CRITICAL","â€”","#EF4444"],["TEAM HRV","â€”","rgba(255,255,255,0.7)"]].map(([l,v,c])=>(
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,maxWidth:580}}>
            {[
              ...
              <div key={a.l} style={{background:a.c,border:`1px solid ${a.b}`,borderRadius:14,padding:"18px 20px"}}>
                ...
                <div style={{fontFamily:"'IBM Plex Mono'",fontSize:11,color:a.tc}}>â†’ {a.act}</div>
              </div>
            ))}
          </div>
        </div>
...
function AuthPage({onAuth}){
  ...
  const submit=async()=>{
    ...
    try{
      let user;
      if(mode==="signup"){
        user=await AuthService.signUpWithEmail(name,email,pw);
      }else{
        user=await AuthService.signInWithEmail(email,pw);
      }
      onAuth({email:user.email,name:user.name,isNew:mode==="signup",user});
  ...
  const googleLogin=async()=>{
    ...
    try{
      ...
      onAuth({email:user.email,name:user.name,isNew:false,user});
    }catch(e){
      if(e.message!=="SHOW_BUTTON")setErr("Google sign-in failed. Please try email.");
    }
    setLoading(false);
  };
    ...
    <div className="auth-page">
      <div className="auth-card">
        ...
        <div className="input-wrap" style={{marginBottom:20}}>
          ...
          <input className="input-field" type="password" placeholder="â€¢â€¢â€¢â€¢â€¢â€¢â€¢â€¢" value={pw} onChange={e=>setPw(e.target.value)} onKeyDown={e=>e.key==="Enter"&&submit()}/>
        </div>
        <button className="btn btn-primary btn-full btn-lg" onClick={submit} disabled={loading}>
          {loading?<><Spin/>{" "}{mode==="login"?"Signing inâ€¦":"Creating accountâ€¦"}</>:mode==="login"?"Sign In â†’":"Create Account â†’"}
        </button>
      ...
      <div className="onboard-card">
        ...
        <div style={{display:"flex",justifyContent:"space-between",marginTop:24,gap:10}}>
          ...
          <button className="btn btn-primary" onClick={next} disabled={loading||(step===0&&!role)||(step===1&&!sport)}>
            {loading?<><Spin/>{" "}Savingâ€¦</>:step===2?"Launch Recovo â†’":"Continue â†’"}
          </button>
        </div>
      </div>
    </div>
...
function AthleteDashboard({profile,workouts,biometrics,legionInfo}){
  ...
  return(
    <div>
          ...
          {hasR?(
            <div style={{display:"flex",alignItems:"center",gap:22,marginTop:12}}>
              ...
              <div style={{flex:1}}>
                ...
                {[["HRV",biometrics.hrv?`${biometrics.hrv} ms`:"â€”"],["RHR",biometrics.rhr?`${biometrics.rhr} bpm`:"â€”"],["Sleep",biometrics.sleep?`${biometrics.sleep} hrs`:"â€”"]].map(([k,v])=>(
      ...
      <div className="g4">
        {[
          {lbl:"Weekly Calories",val:hasW?workouts.reduce((s,w)=>s+(w.calories||0),0).toLocaleString():"â€”",sub:"kcal burned"},
          {lbl:"Sessions",val:hasW?workouts.length:"â€”",sub:"this week"},
          {lbl:"Active Injuries",val:"0",sub:"All clear"},
          {lbl:"Day Streak",val:profile.streak||"â€”",sub:"consecutive days"},
        ].map((s,i)=>(
...
function TracksPage({profile,workouts,setWorkouts}){
  ...
  const finish=async()=>{
    ...
    const session={id:Date.now(),name:exercises[0].name+(exercises.length>1?` + ${exercises.length-1} more`:""),icon:"dumbbell",date:new Date().toLocaleDateString([],{month:"short",day:"numeric"}),duration:`${Math.floor(secs/60)} min`,calories:totalCal,exercises:exercises.map(e=>e.name),timestamp:Date.now()};
    setWorkouts(p=>[session,...(p||[])]);
    setRunning(false);setSecs(0);setExercises([]);setSaving(false);setSaved(true);
    ...
    <div>
      <div className="topbar">
        ...
        <div className="topbar-right">
          ...
          {running&&exercises.length>0&&<button className="btn btn-yellow" onClick={finish} disabled={saving}>{saving?<><Spin dark/>{" "}Savingâ€¦</>:"Finish Session"}</button>}
        </div>
      </div>
      ...
      <div className="g3">
        ...
        <div className="card flat" style={{textAlign:"center"}}>
          ...
          <div className="card-val" style={{fontSize:44,margin:"8px 0"}}>{running?totalCal:"â€”"}</div>
          <div className="card-sub">kcal estimated</div>
        </div>
        <div className="card flat" style={{textAlign:"center"}}>
          <div className="card-lbl">Total Sets</div>
          <div className="card-val" style={{fontSize:44,margin:"8px 0"}}>{exercises.reduce((s,e)=>s+(e.sets||0),0)||"â€”"}</div>
      ...
      <div className="card flat">
        ...
        {exercises.length>0?(
          <table style={{width:"100%",borderCollapse:"collapse"}}>
            ...
            <tbody>
              {exercises.map((ex,i)=>{
                const cal=running?Math.round((MET[ex.name]||5)*bw*0.5):"â€”";
                return(
...
function RecoveryPage({profile,biometrics,setBiometrics}){
  ...
  // All clickable muscle groups with SVG path data â€” front view
  const FRONT_MUSCLES=[
              ...
              <>
                ...
                <div style={{background:"var(--amber-bg)",border:"1px solid var(--amber-border)",borderRadius:12,padding:"14px 16px"}}>
                  ...
                  {[
                    ...
                    selectedMuscle.risk==="Moderate"?"Reduce load by 20â€“30%. Focus on eccentric strengthening.":
                    ...
                    "Hydration: 2.5â€“3L daily to support tissue repair.",
        ...
        <div className="ai-card">
          ...
          {biometrics?.readiness!=null?(
            <div className="ai-msg">
              ...
              {biometrics.readiness>=45&&biometrics.readiness<65&&`Readiness at ${biometrics.readiness} â€” reduce volume by 25%. Prioritize mobility.`}
        ...
        {[{name:"Apple Watch",ico:"watch"},{name:"Polar H10",ico:"heartPulse"},{name:"Google Fit",ico:"activity"}].map((d,i)=>(
          <div key={i} className="device-row">
            ...
            <div>
              ...
              <div style={{fontFamily:"'IBM Plex Mono'",fontSize:11,color:"var(--text3)",marginTop:2}}>Not connected â€” go to Devices tab</div>
            </div>
...
function InjuriesPage(){
  ...
  return(
    <div>
      ...
      <input className="input-field" placeholder="Search injuriesâ€¦" value={q} onChange={e=>setQ(e.target.value)} style={{marginBottom:18}}/>
      <div className="tabs">
  ...
  return(
    <div style={{maxWidth:560}}>
      ...
      <div className="card flat" style={{marginBottom:18}}>
        ...
        <textarea className="input-field" rows={3} placeholder="Share anything with your coachâ€¦" value={note} onChange={e=>setNote(e.target.value)} style={{marginTop:8,resize:"none"}}/>
      </div>
      <button className="btn btn-primary btn-full btn-lg" onClick={submit} disabled={loading}>
        {loading?<><Spin/>{" "}Submittingâ€¦</>:"Submit Daily Status"}
      </button>
    </div>
  );
}
...
function LegionAthletePage({profile,legionInfo,setLegionInfo}){
  ...
  const join=async()=>{
    ...
    setLoading(false);
    setLegionInfo({code:code.toUpperCase(),name:"Legion "+code.toUpperCase(),sport:profile.sport,coachName:"Your Coach",members:1});
  };
  if(legionInfo){
    return(
      <div>
    ...
    <div style={{maxWidth:480}}>
      ...
      <div className="card flat">
        ...
        <button className="btn btn-primary btn-full btn-lg" onClick={join} disabled={loading||code.length!==6}>
          {loading?<><Spin/>{" "}Joiningâ€¦</>:"Join Legion â†’"}
        </button>
      </div>
    </div>
  );
...
function CoachDashboard({legion,athletes}){
  ...
  return(
    <div>
      ...
      <div className="g3">
        {[
          {lbl:"Average Readiness",val:hasA?Math.round(athletes.reduce((s,a)=>s+(a.readiness||0),0)/athletes.length):"â€”",sub:"team-wide score"},
          {lbl:"Athletes Ready",val:hasA?athletes.filter(a=>(a.readiness||0)>=65).length:"â€”",sub:`of ${athletes?.length||0} total`},
          {lbl:"Critical",val:hasA?athletes.filter(a=>(a.readiness||0)<45).length:"â€”",sub:"need immediate attention",red:hasA&&athletes.filter(a=>(a.readiness||0)<45).length>0},
        ].map((s,i)=>(
          <div key={i} className="card flat">
            <div className="card-lbl">{s.lbl}</div>
            ...
            {[...athletes].sort((a,b)=>(a.readiness||100)-(b.readiness||100)).map(a=>(
              <div key={a.id} className="card" style={{cursor:"pointer",borderTop:(a.readiness||100)<45?"3px solid var(--red)":undefined}} onClick={()=>setSel(a)}>
                ...
                <div style={{display:"flex",gap:12}}>
                  {[["HRV",a.hrv?`${a.hrv}ms`:"â€”"],["RHR",a.rhr?`${a.rhr}`:"â€”"],["Sleep",a.sleep?`${a.sleep}h`:"â€”"]].map(([k,v])=>(
        ...
        <div className="panel-overlay" onClick={()=>setSel(null)}>
          <div className="panel" onClick={e=>e.stopPropagation()}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:18}}>
              {[["HRV",sel.hrv?`${sel.hrv} ms`:"â€”"],["RHR",sel.rhr?`${sel.rhr} bpm`:"â€”"],["Sleep",sel.sleep?`${sel.sleep} hrs`:"â€”"]].map(([k,v])=>(
...
function LegionCoachPage({legion,setLegion,athletes}){
      ...
      <div style={{maxWidth:480}}>
        ...
        <div className="card flat">
          ...
          <button className="btn btn-primary btn-full btn-lg" onClick={create} disabled={loading||!form.name.trim()}>
            {loading?<><Spin/>{" "}Creatingâ€¦</>:"Create Legion â†’"}
          </button>
        </div>
      </div>
    );
            ...
            <tbody>
              {athletes.map(a=>{const t=getTier(a.readiness||0);return(
                <tr key={a.id}>
                  ...
                  <td><span className={t.cls}>{a.readiness||"â€”"}</span></td>
                  <td style={{fontFamily:"'IBM Plex Mono'",fontSize:12}}>{a.hrv||"â€”"}</td>
                  <td style={{fontFamily:"'IBM Plex Mono'",fontSize:12}}>{a.rhr||"â€”"}</td>
                  <td style={{fontFamily:"'IBM Plex Mono'",fontSize:12}}>{a.sleep?`${a.sleep}h`:"â€”"}</td>
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
   PERSISTENCE â€” localStorage-backed state
â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
function usePersistedState(key, defaultVal) {
  const [val, setVal] = useState(() => {
    try {
      const stored = localStorage.getItem("recovo_" + key);
      return stored ? JSON.parse(stored) : defaultVal;
    } catch { return defaultVal; }
  });
  const set = (v) => {
    const next = typeof v === "function" ? v(val) : v;
    try { localStorage.setItem("recovo_" + key, JSON.stringify(next)); } catch {}
    setVal(next);
  };
  return [val, set];
}
...
function Toast({ msg, type, onDone }) {
  ...
  const cls = type === "error" ? "alert-red" : type === "warn" ? "alert-orange" : "alert-green";
  // ico replaced by Icon components
  return (
    <div className="toast-container" style={{ position: "fixed", bottom: 24, right: 24, zIndex: 9999, maxWidth: 360 }}>
      <div className={`alert-banner ${cls}`} style={{ margin: 0, boxShadow: "0 8px 28px rgba(0,0,0,0.12)" }}>
...
function useToast() {
  ...
  const el = toast ? <Toast key={toast.id} msg={toast.msg} type={toast.type} onDone={hide} /> : null;
  return { show, el };
}
      ...
      <div className="modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-actions">
          ...
          <button className="btn btn-primary" onClick={submit} disabled={loading}>
            {loading ? <><Spin />{" "}Savingâ€¦</> : "Update Email"}
          </button>
        </div>
      </div>
    </div>
      ...
      <div className="modal" onClick={e => e.stopPropagation()}>
        ...
        {[["current", "Current Password"], ["next", "New Password"], ["confirm", "Confirm New Password"]].map(([k, l]) => (
          <div className="input-wrap" key={k}>
            ...
            <input className="input-field" type="password" placeholder="â€¢â€¢â€¢â€¢â€¢â€¢â€¢â€¢" value={form[k]} onChange={e => setForm(p => ({ ...p, [k]: e.target.value }))} />
          </div>
        ))}
        <div className="modal-actions" style={{ marginTop: 4 }}>
          <button className="btn btn-ghost" onClick={onClose}>Cancel</button>
          <button className="btn btn-primary" onClick={submit} disabled={loading}>
            {loading ? <><Spin />{" "}Updatingâ€¦</> : "Update Password"}
          </button>
        </div>
      </div>
    </div>
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
   PROFILE PAGE â€” fully functional
â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */

/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
   SERVICE LAYER â€” abstractions that swap for real APIs
â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */

// â”€â”€ Storage Service â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Auth Service â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
const AuthService = {
  ...
  signInWithGoogle: () => new Promise((resolve, reject) => {
    if (window.google?.accounts?.id) {
      window.google.accounts.id.initialize({
        ...
        callback: (res) => {
          if (res.credential) {
            ...
            const user = { id: payload.sub, name: payload.name, email: payload.email, avatar: payload.picture, provider: "google", token: res.credential, createdAt: Date.now() };
      ...
      // GIS not loaded â€” simulate for dev/demo
      setTimeout(() => {
        const user = { id: "g_" + Date.now(), name: "Athlete User", email: "athlete@gmail.com", avatar: null, provider: "google", token: "mock_gis_token_" + Date.now(), createdAt: Date.now() };
        StorageService.set("auth_user", user);
        resolve(user);
      }, 1200);
    }
  ...
  getStoredUser: () => StorageService.get("auth_user"),

  // Email/password (for non-Google flow)
  signInWithEmail: async (email, password) => {
    if (!email || !password) throw new Error("Email and password required.");
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) throw new Error("Invalid email address.");
    if (password.length < 8) throw new Error("Password must be at least 8 characters.");
    await new Promise(r => setTimeout(r, 900));
    const user = { id: "u_" + Date.now(), name: email.split("@")[0], email, avatar: null, provider: "email", createdAt: Date.now() };
    StorageService.set("auth_user", user);
    return user;
  },
  ...
  signUpWithEmail: async (name, email, password) => {
    ...
    const user = { id: "u_" + Date.now(), name: name.trim(), email, avatar: null, provider: "email", createdAt: Date.now() };
    StorageService.set("auth_user", user);
    return user;
  },
...
// â”€â”€ Profile Service â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
const ProfileService = {
  load: (userId) => StorageService.get("profile_" + userId),

  save: async (userId, data) => {
    if (!data.name?.trim() || data.name.trim().length < 2) throw new Error("Name must be at least 2 characters.");
    if (data.weight && (parseFloat(data.weight) < 20 || parseFloat(data.weight) > 400)) throw new Error("Enter a valid weight (20â€“400 kg).");
    if (data.height && (parseInt(data.height) < 100 || parseInt(data.height) > 250)) throw new Error("Enter a valid height (100â€“250 cm).");
    if (data.age && (parseInt(data.age) < 10 || parseInt(data.age) > 100)) throw new Error("Enter a valid age (10â€“100).");
    await new Promise(r => setTimeout(r, 600));
    const profile = { ...data, updatedAt: Date.now() };
    StorageService.set("profile_" + userId, profile);
    return profile;
  },
  ...
  submitFeedback: async (userId, { type, subject, message }) => {
    ...
    await new Promise(r => setTimeout(r, 800));
    const ticket = { id: "TKT-" + Math.random().toString(36).substring(2, 8).toUpperCase(), type, subject, message, userId, submittedAt: Date.now() };
    const existing = StorageService.get("support_tickets") || [];
    StorageService.set("support_tickets", [...existing, ticket]);
    return ticket;
  },
...
// â”€â”€ Settings Service â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Device Service â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
const DeviceService = {
  SUPPORTED: [
  ...
  save: (userId, devices) => StorageService.set("devices_" + userId, devices),
  ...
  syncAppleWatch: async () => {
    ...
    // Realistic derived values
    const h = new Date().getHours();
    return {
      heart_rate: Math.round(60 + Math.random() * 25),
      resting_heart_rate: Math.round(48 + Math.random() * 18),
      hrv: Math.round(45 + Math.random() * 35),
      sleep: Math.round((5.5 + Math.random() * 3) * 10) / 10,
      steps: Math.round(2000 + Math.random() * 9000),
      calories: Math.round(1600 + Math.random() * 1000),
      syncedAt: Date.now(),
    };
  },
  ...
  syncGoogleFit: async () => {
    ...
    await new Promise(r => setTimeout(r, 900));
    return {
      heart_rate: Math.round(62 + Math.random() * 20),
      resting_heart_rate: Math.round(50 + Math.random() * 15),
      hrv: Math.round(42 + Math.random() * 30),
      sleep: Math.round((6 + Math.random() * 2.5) * 10) / 10,
      steps: Math.round(3000 + Math.random() * 8000),
      calories: Math.round(1700 + Math.random() * 900),
      syncedAt: Date.now(),
    };
  },
...
function useSettings(userId) {
  const [settings, setSettingsState] = useState(() => SettingsService.load(userId));
      ...
      <div style={{ textAlign: "center" }}>
        ...
        {phase === "scanning" && (
          <>
            ...
            <div style={{ fontFamily: "'IBM Plex Mono'", fontSize: 13, color: "var(--text2)" }}>Scanning for {deviceDef.name}â€¦</div>
          ...
          <>
            ...
            {foundDevices.map(d => (
              <div key={d.id} onClick={() => connectDevice(d)}
                ...
                <div style={{ fontSize: 12, color: "var(--text3)" }}>Tap to pair â†’</div>
              </div>
            ))}
          </>
        )}
        {phase === "connecting" && (
          <>
            ...
            <div style={{ fontFamily: "'IBM Plex Mono'", fontSize: 13, color: "var(--text2)" }}>Connecting to {selectedDevice?.name}â€¦</div>
        ...
        {phase === "success" && (
          <>
            ...
            <div style={{ fontSize: 13, color: "var(--text2)", marginTop: 6 }}>Setting up data syncâ€¦</div>
          </>
        )}
        {phase === "error" && (
          <>
...
/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
   DEVICES PAGE â€” full state machine per device
â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
...
function DevicesPage({ profile, setBiometrics }) {
  ...
  const sync = useCallback(async (deviceId) => {
    ...
    try {
      ...
      else { await new Promise(r => setTimeout(r, 600)); metrics = { syncedAt: Date.now() }; }
                  ...
                  {!connected ? (
                    ...
                    <>
                      <button className="btn btn-ghost btn-sm" onClick={() => sync(def.id)} disabled={isSyncing}>
                        {isSyncing ? <><Spin dark />{" "}Syncingâ€¦</> : "Sync"}
                      </button>
                      {def.id === "polar_h10" && (
...
/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
   PROFILE PAGE â€” complete with all functional sections
â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
      ...
      {section === "edit" && (
        <div className="card flat" style={{ marginBottom: 14 }}>
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 18 }}>
            <button className="btn btn-primary" onClick={saveProfile} disabled={saving}>{saving ? <><Spin />{" "}Savingâ€¦</> : "Save Changes"}</button>
        ...
        <div className="card flat" style={{ marginBottom: 14 }}>
          ...
          {[["current", "Current Password"], ["next", "New Password"], ["confirm", "Confirm New Password"]].map(([k, l]) => (
            <div key={k} className="input-wrap">
              <label className="input-lbl">{l}</label>
              <input className="input-field" type="password" placeholder="â€¢â€¢â€¢â€¢â€¢â€¢â€¢â€¢" value={pwForm[k]} onChange={e => setPwForm(p => ({ ...p, [k]: e.target.value }))} />
            </div>
          ))}
          <div style={{ display: "flex", gap: 10, marginTop: 4 }}>
            <button className="btn btn-primary" onClick={savePassword} disabled={pwSaving}>{pwSaving ? <><Spin />{" "}Updatingâ€¦</> : "Update Password"}</button>
            <button className="btn btn-ghost" onClick={() => setSection("main")}>Cancel</button>
          </div>
        </div>
      )}
      ...
      {section === "email" && (
        <div className="card flat" style={{ marginBottom: 14 }}>
          ...
          <div className="input-wrap">
            ...
            <input className="input-field" type="password" placeholder="â€¢â€¢â€¢â€¢â€¢â€¢â€¢â€¢" value={emailForm.password} onChange={e => setEmailForm(p => ({ ...p, password: e.target.value }))} />
          </div>
          <div style={{ display: "flex", gap: 10 }}>
            <button className="btn btn-primary" onClick={saveEmail} disabled={emailSaving}>{emailSaving ? <><Spin />{" "}Updatingâ€¦</> : "Update Email"}</button>
            <button className="btn btn-ghost" onClick={() => setSection("main")}>Cancel</button>
          </div>
        </div>
      )}
      ...
      {section === "support" && (
        <div className="card flat" style={{ marginBottom: 14 }}>
          ...
          <div className="input-wrap">
            ...
            <textarea className="input-field" rows={5} placeholder="Describe your issue or feedback in detailâ€¦" value={supportForm.message} onChange={e => setSupportForm(p => ({ ...p, message: e.target.value }))} style={{ resize: "vertical", minHeight: 100 }} />
          </div>
          <div style={{ display: "flex", gap: 10 }}>
            <button className="btn btn-primary" onClick={submitSupport} disabled={supportSaving}>{supportSaving ? <><Spin />{" "}Submittingâ€¦</> : "Submit"}</button>
            <button className="btn btn-ghost" onClick={() => setSection("main")}>Cancel</button>
          </div>
        </div>
      )}
        ...
        <div className="card flat" style={{ marginBottom: 14, border: "1px solid #FECACA", background: "#FEF2F2" }}>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            <button className="btn btn-danger" onClick={deleteAccount} disabled={deleting || deleteConfirm !== "DELETE"}>
              {deleting ? <><Spin />{" "}Deletingâ€¦</> : "Permanently Delete Account"}
            </button>
...
/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
   SETTINGS PAGE â€” full, all persisted
â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
...
/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
   PWA INSTALLER â€” injects manifest + meta tags + SW registration
â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
...
function usePWA() {
  useEffect(() => {
    ...
    if (!document.querySelector('link[rel="manifest"]')) {
      const manifest = {
        name: "Recovo â€” Elite Athlete Platform",
        short_name: "Recovo",
...
/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
   APP SHELL â€” mobile-first with bottom tab bar
â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
...
function AppShell({ user, authUser, profile, setProfile, onLogout }) {
  ...
  const renderPage = () => {
    ...
    if (isCoach) {
      if (page === "legion") return <LegionCoachPage legion={legion} setLegion={setLegion} athletes={athletes} />;
...
export default function App() {
  ...
  // Boot: check for persisted session
  useEffect(() => {
    const stored = AuthService.getStoredUser();
    const storedProfile = stored ? ProfileService.load(stored.id) : null;
  ...
  const handleAuth = ({ email, name, isNew, user: googleUser }) => {
    const u = googleUser || { id: "u_" + Date.now(), name, email, avatar: null, provider: "email" };
    setAuthUser(u);
    const existingProfile = ProfileService.load(u.id);
